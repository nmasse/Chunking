...
def plot_pev_cross_time(x, num_pulses, cue, pev_type):
    ...
    plt.savefig("./savedir/chunking_8_cue_off/"+pev_type+"_cross_time_"+str(num_pulses)+"_pulses_"+cue+".png")
...
def plot_pev_after_stim(x, num_pulses, cue, pev_type,time_lapse):
    ...
    plt.savefig("./savedir/chunking_8_cue_off/"+pev_type+"_after_stim_"+str(num_pulses)+"_pulses_"+cue+".png")
    plt.close()
    plt.figure()
    ...
    plt.savefig("./savedir/chunking_8_cue_off/"+pev_type+"_selectivity_"+str(num_pulses)+"_pulses_"+cue+".png")
    plt.close()
...
def shufffle_pev(x, num_pulses, cue, pev_type, acc_type, time_lapse):
    ...
    for ax in axes.flat:
        ...
        i += 1
    plt.savefig("./savedir/chunking_8_cue_off/"+pev_type+"_shuffle_"+str(num_pulses)+"_pulses_"+cue+".png")
    ...
    for num_pulses in num_pulses:
        for cue in cue_list:
            for i in range(len(pev_type)):
                    x = pickle.load(open('./savedir/cut_accuracy_chunking_'+str(num_pulses)+"_"+cue+".pkl", 'rb'))
