...
def plot_pev_cross_time(x, num_pulses, cue, pev_type):
    ...
    i = 0
    for ax in axes.flat:
        im = ax.imshow(x[pev_type][:,i,:,0],aspect='auto')
        #im = ax.imshow(np.mean(x[pev_type][:,i,:,:],axis=2), aspect='auto')
        i += 1
    cax,kw = mpl.colorbar.make_axes([ax for ax in axes.flat])
    plt.colorbar(im, cax=cax, **kw)
    plt.savefig("./savedir/original/"+pev_type+"_cross_time_"+str(num_pulses)+"_pulses_"+cue+".png")
...
def plot_pev_after_stim(x, num_pulses, cue, pev_type,time_lapse):
    ...
    for ax in axes.flat:
        ...
        im = ax.imshow(x[pev_type][:,:,x['timeline'][2*i+1]+time_lapse,0],aspect='auto')
        i += 1
    cax,kw = mpl.colorbar.make_axes([ax for ax in axes.flat])
    plt.colorbar(im, cax=cax, **kw)
    eolongd = (par['dead_time']+par['fix_time'] + num_pulses * par['sample_time'] + (num_pulses-1)*par['delay_time'] + par['long_delay_time'])//par['dt']
    #plt.imshow(x[pev_type][:,:,eolongd-time_lapse],aspect='auto')
    plt.savefig("./savedir/original/"+pev_type+"_after_stim_"+str(num_pulses)+"_pulses_"+cue+".png")
    plt.close()
    plt.figure()
    m = np.array(x[pev_type][:,:,eolongd-time_lapse,0])
    plt.hist(np.sum((m>0.1),axis=1), bins=range(9))
    plt.xticks(range(9))
    plt.xlim(xmin=0, xmax=9)
    plt.savefig("./savedir/original/"+pev_type+"_selectivity_"+str(num_pulses)+"_pulses_"+cue+".png")
    plt.close()
...
def shufffle_pev(x, num_pulses, cue, pev_type, acc_type, time_lapse):
    ...
    for ax in axes.flat:
        ...
        i += 1
    plt.savefig("./savedir/original/"+pev_type+"_shuffle_"+str(num_pulses)+"_pulses_"+cue+".png")
