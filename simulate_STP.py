...
def run_sim_step(x, u, alpha_std, alpha_stf, U, spike):
    ...
    x = np.minimum(np.complex64(1), np.maximum(np.complex64(0), x)) # All complex64 here changed from float32
    u = np.minimum(np.complex64(1), np.maximum(np.complex64(0), u)) # All complex64 here changed from float32
